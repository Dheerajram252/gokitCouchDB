...
var (
	...
	serverTimeout = flag.Int64("server.timeout", 5000000, "server time out in milliseconds")
	sysLogAddress = flag.String("syslog.address", "localhost:514", "default location for the sysLogger")
	dbName        = flag.String("db.name", "user", "persistance db name")
	dbType        = flag.String("db.type", "couch", "persistance db type")
)
...
func initApp() (log.Logger, base.Service, error) {
	...
	client, registrar, err := initRegister(logger)
	if err != nil {
		return logger, nil, err
	}
	registrar.Register()
	dbs := initDatabases(client, logger)
	s := initService(logger, dbs)
	return logger, s, nil
}
...
func initDatabases(client *api.Client, logger log.Logger) *base.RoundRobin {
	dbInstancer := consul.NewInstancer(
		...
		logger,
		fmt.Sprintf("db-%s-%s", *basePath, *dataType),
		[]string{},
		true,
	)
